...
1. **M4 — Co-op combat loop:** implement team attack, then run all solo and 2–3 player Stage 1 combinations to close the M4 exit gate.
...
- Character selection, typed role tuning, and complete original 24-state sheets exist for all four heroes. Their timing contract remains shared until later role-balance passes. Three-slot local device routing, runtime join/leave, safe spawning, active-player targeting, shared-camera framing, independent selection/ready states, compact player HUDs, faction-safe combat ownership, spawn-time co-op scaling, teammate rescue, and per-player continues exist; team attack remains the final M4 gameplay-system batch. Vehicle and high-score persistence do not exist yet; the procedural Stage 1 score and three-family weapon set are foundations for the final soundtrack and weapon sandbox rather than complete campaign coverage.
