...
func run(test) -> void:
	...
	test.check(game.confirm_hero_selection_for_slot(2) and game.state == "campaign_map", "all ready local players did not reach the campaign route together")
	game._deploy_campaign_stage()
	test.check(game.state == "playing", "campaign route did not deploy the ready local team")
