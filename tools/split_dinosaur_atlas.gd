...
const CONTENT_LIMIT := Vector2i(292, 276)
...
func _init() -> void:
	...
	if args.size() != 4:
		push_error("usage: -- <source.png> <row-0.png> <row-1.png> <row-2.png>")
		quit(2)
		return
	var source := Image.load_from_file(ProjectSettings.globalize_path(args[0]))
	if source == null:
		push_error("failed to load source atlas: %s" % args[0])
		quit(3)
		return
	for row in range(ROWS):
		var output := Image.create(CELL_SIZE.x * COLUMNS, CELL_SIZE.y, false, Image.FORMAT_RGBA8)
		output.fill(Color.TRANSPARENT)
		var row_rect := _row_rect(source.get_size(), row)
		var source_row := source.get_region(row_rect)
		_threshold_alpha(source_row)
		...
		if components.size() < COLUMNS:
			push_error("row %d contains only %d isolated sprites" % [row, components.size()])
			quit(5)
			return
		...
		if save_error != OK:
			push_error("failed to save dinosaur row: %s" % args[row + 1])
			quit(4)
			return
		print("DINOSAUR_ROW_EXPORTED row=%d components=%d path=%s" % [row, components.size(), args[row + 1]])
	quit()
...
func _threshold_alpha(image: Image) -> void:
	for y in range(image.get_height()):
		for x in range(image.get_width()):
			var pixel := image.get_pixel(x, y)
			if pixel.a < ALPHA_THRESHOLD or maxf(pixel.r, maxf(pixel.g, pixel.b)) <= 0.005:
				pixel.a = 0.0
