...
Post-release evidence (2026-08-22, pickup-art refresh): all twelve weapon drops and all eight food/treasure drops replace their generic procedural primitives with twenty independent, clean-room arcade pixel-art models. A retained deterministic builder removes the isolation checker, rejects neighboring-cell fragments, reorders generated cells to stable catalog IDs, and normalizes the accepted artwork into two transparent 160-pixel-cell runtime atlases. Regression coverage verifies every explicit pickup ID owns one unique cell, preserves the legacy food/weapon-family aliases, and keeps both atlas dimensions exact; archived generation canvases are excluded from Web and iOS packages. Fifty-three suites pass 3,984/3,984 assertions. Exported weapon and item fixtures were visually accepted at desktop and 844×390 mobile-landscape scale with no cell bleed, crop loss, label overlap, or browser warnings/errors. Web export, iOS project export, and unsigned generic-device Mach-O arm64 build pass.
...
Post-release evidence (2026-08-22, held-weapon strike-alignment maintenance): exported QA reproduced the remaining oddness as two independent defects: every padded 160×120 pickup cell was stretched into a square with a blanket firearm flip, while the hero's baked machete/pistol frames were rendered underneath the production held model, creating visible duplicate weapons and a waist-level firing origin. All twelve weapon resources now own a cropped source rectangle, grip pivot, preserved scale, source-facing direction, idle angle, and deterministic contact angle. The runtime interpolates the weapon from the normalized hand position through the real attack hit frame and recovery, uses unarmed body poses so no baked duplicate survives, renders melee grips behind the hand and ranged models in front, and keeps the shock effect attached to the baton tip. Armed melee strikes no longer enter the unarmed contact-grab state. Machete contact, shotgun contact/idle, and rocket contact fixtures pass rendered acceptance; the latest Web samples sustain approximately 120 FPS across 300 frames with zero frames above 20/33 ms and no browser warnings/errors. Fifty-three suites pass 4,057 assertions.
...
- **2026-08-22 — Held-weapon animation contract:** every weapon owns authored crop, grip, scale, source-facing, idle, and contact-pose metadata; the body uses weapon-free combat frames while the held model follows the same deterministic contact timing as its hitbox, and armed strikes cannot trigger unarmed grabs.
