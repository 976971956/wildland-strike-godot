...
1. **M3 — Stage 1 presentation:** finish the remaining SFX/UI polish and final audio hierarchy.
2. **M3 — Stage 1 exit gate:** run a complete balance, device, visual, audio, and stage-clear acceptance pass.
3. **M4 — Hero/co-op foundation:** introduce the remaining playable-hero data model and shared local co-op camera/input architecture.
...
- Stage 1 now has independent final-direction background art, per-scene foreground motion, original state-driven music, a 24-frame Ranger sheet, eight-state enemy atlases, and a complete staged score settlement, but still needs final SFX mix and HUD polish before its exit gate.
