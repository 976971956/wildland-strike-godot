...
1. **M3 — Stage 1 presentation:** expand grunt, brute, hunter, raptor, and boss state animation coverage, then finish the remaining SFX/UI polish.
...
- Ranger now has distinct state and weapon frames; enemies still switch between four frames per archetype and share art across several behavior phases.
...
- Stage 1 now has independent final-direction background art, per-scene foreground motion, original state-driven music, a 24-frame Ranger sheet, and a complete staged score settlement, but still needs expanded enemy state animation.
