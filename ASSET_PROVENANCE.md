...
- `assets/sprites/enemy_sheet.png`: 1448×1086 PNG; SHA-256 `7665155d3080d3cc976732d2f60e5b6fdd81406ca024588a308eebe181c5a5c5`. Early clean-room enemy concept sheet with no external screenshot, ROM, logo, or traced frame. It is superseded by the individual production sheets and excluded from every runtime export.
- `assets/sprites/raptor_sheet.png`: 2079×756 PNG; SHA-256 `b5ce0d1a249a1d7cc0fd56122e663d77b2cfa5decf23a93edd8545cfebdabd95`. Early clean-room raptor generation canvas with no external screenshot, ROM, logo, or traced frame. It is superseded by `raptor_sheet_v2.png` and excluded from every runtime export.

The remaining early baseline assets are already named in their downstream production entries: `ruined_city_stage.png` as the original Stage 1 environment, `ranger_sheet.png` as the original Ranger identity source, and their accepted regenerated/expanded derivatives. `assets/app_icon.svg` is a project-authored code-native vector mark (SHA-256 `23f3821be647c94bd3f8218b044ca5946b7d10e4f80746d10b78bea415326e4e`) containing only the original `WS` monogram and claw-strike geometry.

All music, sound effects, effort voices, particles, HUD primitives, telegraphs, and procedural world overlays are synthesized or drawn at runtime by project source; no third-party audio file is shipped. Noto Sans SC is the only third-party content asset and is separately identified with its upstream URL, checksum, and complete OFL 1.1 text in `assets/fonts/`.

Final audit result: every tracked PNG is named in this log; all archived generation/edit/prototype canvases are excluded from both Web and iOS; the release tree contains no ROM, emulator image, sprite rip, protected franchise name, external music/dialogue, or copied logo. Generated output ownership and public relicensing were checked against OpenAI's current content terms, which assign available output rights to the user, while the project remains responsible for non-infringement: https://openai.com/policies/may-2025-business-terms/. Original project assets are released under CC BY 4.0; code/data/tests/tools/docs are MIT; third-party terms are consolidated in `LICENSE` and `THIRD_PARTY_NOTICES.md`.
