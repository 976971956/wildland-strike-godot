...
1. **M3 — Stage 1 exit gate:** run a complete balance, device, visual, audio, and stage-clear acceptance pass; fix every critical/high defect before closing the milestone.
2. **M4 — Hero/co-op foundation:** introduce the remaining playable-hero data model and shared local co-op camera/input architecture.
...
- Stage 1 now has independent final-direction background art, per-scene foreground motion, original state-driven music and SFX hierarchy, a 24-frame Ranger sheet, eight-state enemy atlases, a mobile-safe combat HUD, and a complete staged score settlement; its remaining M3 work is the end-to-end balance/device/acceptance exit pass.
