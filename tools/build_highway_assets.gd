...
const SOURCE_PATH := "res://assets/sprites/iron_vulture_source.png"
const OUTPUT_PATH := "res://assets/sprites/iron_vulture_sheet.png"
const CELL_COUNT := 8
const OUTPUT_CELL := Vector2i(320, 320)
...
func _initialize() -> void:
	...
	for index in range(CELL_COUNT):
		...
		var cell := source.get_region(Rect2i(start_x, 195, end_x - start_x, 330))
		var bounds := _visible_bounds(cell)
		if bounds.size.x <= 0 or bounds.size.y <= 0:
			push_error("Iron Vulture cell %d contains no visible pixels" % index)
			quit(2)
			return
		var isolated := cell.get_region(bounds)
		var scale_factor := minf(292.0 / isolated.get_width(), 238.0 / isolated.get_height())
		...
		var destination := Vector2i(index * OUTPUT_CELL.x + (OUTPUT_CELL.x - target_size.x) / 2, OUTPUT_CELL.y - target_size.y - 24)
